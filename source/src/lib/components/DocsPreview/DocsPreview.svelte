...
<div class="previewer {classesBase}">
    ...
    {#if tabView === 'preview'}
        ...
        {#if swatches}
            <div
                class="previewer-swatches {classesSwatches}"
            ...
            >
                {#each Object.entries(backgrounds) as [k, v], i}
                    <button
                        ...
                        title={k}
                    >
                        {#if i === 0}<i class="fa-regular fa-circle-xmark text-xl" />{/if}
                    </button>
                {/each}
            </div>
        {/if}
        <!-- Viewport -->
        <div class="previewer-viewport {classesViewport} tailwind-gradient">
            <!-- Slot: Lead -->
